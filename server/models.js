...
export const Artisan = db.define('Artisan', {
  ...
  name: DataTypes.STRING(160), rating: DataTypes.DECIMAL(2, 1),
  about: DataTypes.TEXT, email: DataTypes.STRING(254), website: DataTypes.STRING(500),
  is_top: DataTypes.BOOLEAN, specialty_id: DataTypes.SMALLINT.UNSIGNED, city_id: DataTypes.SMALLINT.UNSIGNED,
}, { tableName: 'artisans' });
...
City.hasMany(Artisan, { foreignKey: 'city_id', as: 'artisans' });
Artisan.belongsTo(City, { foreignKey: 'city_id', as: 'city' });
...
export function serializeArtisan(model) {
  ...
  return { id: a.id, name: a.name, rating: Number(a.rating), city: a.city.name, about: a.about,
